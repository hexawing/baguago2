...
> 以十九路经纬，演天地之变数序。
>
> 以下「背景与弈旨」「原案规则」节录自作者原案（知乎专栏《八卦围棋》）；「程序使用说明」「与原案差异」为本 Demo 的实际实现说明。
...
原案此节为带图表格；此处以 Unicode 卦符 + 三爻属性还原为文字表，方便阅读与排版。

每卦点依「三爻定三性」：**上爻=公私（+1 / −1 目），中爻=通气（斜角是否供气），初爻=虚实（可否落子）**。

| 卦 | 符号 | 上爻·公私 | 中爻·通气 | 初爻·虚实 | 程序中效果 |
| --- | --- | --- | --- | --- | --- |
| 乾 | ☰ | 私（+1） | 通气 | 虚（不可落子） | 恒为空；占之 +1 目 |
| 兑 | ☱ | 公（−1） | 通气 | 虚（不可落子） | 恒为空；占之 −1 目 |
| 离 | ☲ | 私（+1） | 不通气 | 虚（不可落子） | 恒为空；占之 +1 目 |
| 震 | ☳ | 公（−1） | 不通气 | 虚（不可落子） | 恒为空；占之 −1 目 |
| 巽 | ☴ | 私（+1） | 通气 | 实（可落子） | 可落子；占之 +1 目 |
| 坎 | ☵ | 公（−1） | 通气 | 实（可落子） | 可落子；占之 −1 目 |
| 艮 | ☶ | 私（+1） | 不通气 | 实（可落子） | 可落子；占之 +1 目 |
| 坤 | ☷ | 公（−1） | 不通气 | 实（可落子） | 可落子；占之 −1 目 |

> 注：**公私、虚实** 两列与原案第六、七节明示一致；**通气（中爻）** 一列沿用本程序当前映射（乾·兑·巽·坎 通气，离·震·艮·坤 不通）。若你原图表中各卦的「中爻」另有指定，我可同步修正代码 `TRIGRAMS.diagonalChi`。
